...
function facebookLoginService(facebookLoginInitializerService, config, $http, $q, userService, sideNavEventService) {
    function loginCallBack(response, deferredObject) {
        $http.post(config.serviceUrl + '/api/fblogin',
            {
    ...
    return {
        ...
        completeRegistration: function (registerData, fbResponse) {
            ...
            $http.post(config.serviceUrl + '/api/fbRegister',
                {
...
module.exports = ["facebookLoginInitializerService", "config", "$http", "$q", "userService", "sideNavEventService", facebookLoginService];