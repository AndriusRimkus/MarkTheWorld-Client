...
function filterMapController($mdDialog, config, $q, $http, mapService, userService, filterMapEventService) {
    var vm = this;
    ...
    vm.showUserSpots = function (username) {
        ...
        $http.get(config.serviceUrl + 'api/username/' + username)
    ...
    vm.querySearch = function (query) {
        ...
        $http.get(config.serviceUrl + '/api/autocomplete/' + query + '/5').then(
            function (success) {
...
module.exports = ['$mdDialog', 'config', '$q', '$http', 'mapService', 'userService', 'filterMapEventService', filterMapController];